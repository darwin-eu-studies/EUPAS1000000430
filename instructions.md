...
2)  Open the project `basename(list.files(pattern = "\\.Rproj$",full.names = TRUE)[1])` from the `studyCode` directory in RStudio (when inside the project, you will see its name on the top-right of your RStudio session)
